// 1+2+3 + ... + n(-2) + n(-1) + n computacao demorada
...
// resultado.then((soma) => {
//     console.log(`Funcionou: ${soma}`)
//     calculoDemorado(soma).then((soma2) => {
//         console.log(`Funcionou(2): ${soma2}`)
//     })
// })
//     .catch(function (err) {
...
const calculoRapido = (n) => {
    // if (n < 0)
    //     return Promise.reject('Número negativo')
    return n >= 0 ? Promise.resolve(n * (n + 1) / 2) : Promise.reject('Numero negativo')
    // return new Promise((resolve, reject) => {
    //     resolve(n * (n + 1) / 2)
    // })
}
const resultado = calculoRapido(10)
resultado.then((soma) => {
    console.log(`Funcionou: ${soma}`)
})
    .catch(function (err) {
